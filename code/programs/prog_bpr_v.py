...
class Params(object):
    ...
    def build_model_one_by_one(self, flag):
        ...
        if 0 == flag:        # Gru
            ...
            if 0.0 != p['fea_random_zero']:
                print('\tAttention: {v1}% whole features are randomly reset to zero.'
                      .format(v1=p['fea_random_zero'] * 100))
            size_total = 2
            model = OboVBpr(
                train=[self.tra_buys_masks, self.tra_masks, self.tra_buys_neg_masks],
                test= [self.tes_buys_masks, self.tes_masks, self.tes_buys_neg_masks],
                alpha_lambda=[p['alpha'], p['lambda'], p['lambda_ev'], p['fea_random_zero']],
                n_user=self.user_num,
                n_item=self.item_num,
                n_in=size,
        ...
        return model, model_name, size_total
...
def train_valid_or_test():
    ...
    for epoch in np.arange(p['epochs']):
        ...
        if epoch > 0:       # epoch=0的负样本已在循环前生成，且已用于类的初始化
            tra_buys_neg_masks = np.asarray(fun_random_neg_tra(item_num, tra_buys_masks))
            tes_buys_neg_masks = np.asarray(fun_random_neg_tes(item_num, tra_buys_masks, tes_buys_masks))
